<template>
  <div class="mx-5">
    ...
    <div class="container mt-14">
      <div class="w-full">
        <h4 class="text-gray-700 text-xl font-medium mt-0 mb-3">Cadastrar Secretário</h4>
...
export default {
  ...
  methods: {
    async createSecretary() {
      this.loading = true;
      const secretary = await User.create({...this.secretary, type: 'S'});
      if (!secretary.success)
        return this.setMessage("error", "Erro!", secretary.message, 3000);

      this.setMessage(
                "success",
                "Sucesso!",
                "Secretário Cadastrado!",
                3000)
